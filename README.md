...
| Hierarchical (random-access) GOP | **Validated (decoder)** | `ra_selfcheck_gate.sh` — 11/11 cells at 256x256 p6 q40 across hierarchical_levels 1..5, complete mini-GOPs plus trailing partial windows: every display frame's reconstruction byte-identical to `aomdec`, with a hidden-frame (`show_existing`) anti-vacuity pin. Stream bytes are NOT claimed identical to C — the generic inter envelope applies |
