...
W = H = 512
QPS = [20, 32, 44]
PRESETS = [-1, 0, 1, 4, 6, 8, 10]
ARM_MAP = {
    -1: ["t1","t2","t3","t4","st"],
     0: ["t1","t2","t3","t4","st"],
     1: ["t1","t2","t3","t4","st"],
     4: ["t1","t3","s0","s3","s5","qm2","qm6","ac10","vs2","cv1"],
     6: ["t1","t3","s0","s3","s5","qm2","qm6","ac10","vs2","cv1"],
     8: ["t1","t3","s0","s3","s5","qm2","qm6","ac10","vs2","cv1"],
    10: ["t1","t3","s0","s3","s5","qm2","qm6","ac10","vs2","cv1"],
}
...
ARMS = {
    ...
    "s0":  {"SVTAV1_STILL_TUNE": "1", "SVT_FORK_SHARPNESS": "0"},
    "s3":  {"SVTAV1_STILL_TUNE": "1", "SVT_FORK_SHARPNESS": "3"},
    "s5":  {"SVTAV1_STILL_TUNE": "1", "SVT_FORK_SHARPNESS": "5"},
    "qm2": {"SVTAV1_STILL_TUNE": "1", "SVT_FORK_MIN_QM_LEVEL": "2"},
    "qm6": {"SVTAV1_STILL_TUNE": "1", "SVT_FORK_MIN_QM_LEVEL": "6"},
    "ac10":{"SVTAV1_STILL_TUNE": "1", "SVT_FORK_AC_BIAS": "1.0"},
    "vs2": {"SVTAV1_STILL_TUNE": "1", "SVT_FORK_VARIANCE_BOOST_STRENGTH": "2"},
    "cv1": {"SVTAV1_STILL_TUNE": "1", "SVT_FORK_VARIANCE_BOOST_CURVE": "1"},
}
...
def main():
    ...
    jobs = [(cls, p, arm, preset, qp)
            for cls, p in rows for preset in PRESETS for arm in ARM_MAP[preset] for qp in QPS]
    print(f"{len(jobs)} cells", file=sys.stderr)
    out = open(OUT / "results_backfill.tsv", "w")
