...
fn bench_dsp(suite: &mut Suite) {
    ...
    pair!(
        "cdef_find_dir_8bit",
        cdef::cdef_find_dir_8bit(src, STRIDE, 0)
    );
    {
...

    {
        ...
        for (name, is_fp) in [
            ("quantize_fp_raster_1024", true),
            ("quantize_b_raster_1024", false),
        ] {
...

    {
        ...
        let coef: &'static [i32] = Box::leak(
            (0..64 * 64)
                .map(|i| (((i * 2654435761u64 as usize) % 4001) as i32) - 2000)
                .collect::<Vec<i32>>()
                .into_boxed_slice(),
        );
        macro_rules! txbench {
