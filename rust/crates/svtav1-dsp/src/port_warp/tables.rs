//! Constant tables for the warped-motion port.
//!
//! Both are TRANSCRIBED FROM the C source and then PINNED against the real C
//! arrays at evidence tier 1 (`tests/c_parity_warp_model.rs`), so a
//! transcription slip fails a test rather than silently changing pixels.

/// `svt_aom_warped_filter` (`Codec/warped_motion.c:57`), the
/// `WARPEDPIXEL_PREC_BITS == 6` arm — the live one. NOTE: the same
/// initializer list in that file also carries an `#elif
/// WARPEDPIXEL_PREC_BITS == 5` arm with 97 rows; reading the wrong arm yields
/// a table of the wrong SHAPE, so the extraction was bounded to the `== 6`
/// block explicitly.
///
/// `WARPEDPIXEL_PREC_SHIFTS * 3 + 1 = 193` phases covering the
/// `[-1, 2)` fractional range at 6-bit sub-pel precision; every row sums to
/// `1 << FILTER_BITS = 128`.
pub const WARPED_FILTER: [[i16; 8]; 193] = [
    [0, 0, 127, 1, 0, 0, 0, 0],
    [0, -1, 127, 2, 0, 0, 0, 0],
    [1, -3, 127, 4, -1, 0, 0, 0],
    [1, -4, 126, 6, -2, 1, 0, 0],
...
/// `div_lut` (`Codec/warped_motion.c`), the `DIV_LUT_NUM + 1 = 257`-entry
/// fixed-point reciprocal table used by `resolve_divisor_32/64`. Entries are
/// `1/x` at `DIV_LUT_PREC_BITS = 14` precision over `x` in `[1, 2)`.
pub const DIV_LUT: [u16; 257] = [
