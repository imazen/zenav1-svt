// AVX-512 (`_v4`) forward 1D DCT kernels — each processes 16 independent
// columns/rows in the lanes of an `i32x16`. Mechanically widened from the
// `_x8` kernels in `txfm_simd_kernels.rs` (same op-for-op butterfly order ->
// bit-identical per lane). Included into `mod v4` (x86_64 + avx512 only).
...
// ---------------------------------------------------------------------------
#[cfg_attr(target_arch = "x86_64", rite(v4))]
pub(super) fn fdct16_x16(
    t: X64V4Token,
    inp: &[__m512i; 16],
    out: &mut [__m512i; 16],
    cos_bit: i8,
...
#[cfg_attr(target_arch = "x86_64", rite(v4))]
pub(super) fn fdct32_x16(t: X64V4Token, inp: &[__m512i; 32], out: &mut [__m512i; 32], cos_bit: i8) {
    let cospi = cospi_arr(cos_bit);
    let rnd = splat(t, 1 << (cos_bit as u32 - 1));
    let sh = _mm_cvtsi32_si128(cos_bit as i32);
    // stage 1
...
#[cfg_attr(target_arch = "x86_64", rite(v4))]
pub(super) fn fdct64_x16(t: X64V4Token, inp: &[__m512i; 64], out: &mut [__m512i; 64], cos_bit: i8) {
    let cospi = cospi_arr(cos_bit);
    let rnd = splat(t, 1 << (cos_bit as u32 - 1));
    let sh = _mm_cvtsi32_si128(cos_bit as i32);
    // stage 1
