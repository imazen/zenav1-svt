...
/// libaom `CDEF_ADAPTIVE` (`--enable-cdef=3`) arm 1: the mapped-qindex
/// `cq_level <= 32` early-off (pickcdef.c:846-857) — CDEF carries one
/// all-zero strength and `cdef_bits = 0`, so the decoder's `do_cdef` is
/// false. `base_qindex` here IS aom's mapped `cq_level` (both are the
/// 0..=255 qindex domain, `av1_quantizer_to_qindex`-mapped). Damping is
/// still the qp-derived value the searched path would have signaled —
/// aom derives it before the early return.
///
/// This is a Zen-extension pick, not a C-SVT state: SVT v4.2.0 has no
/// equivalent knob. It must only be reached through
/// [`crate::enhancements::ZenEnhancement::AomAdaptiveCdef`].
#[must_use]
pub fn aom_adaptive_cdef_off_pick(base_qindex: u8) -> CdefPick {
    CdefPick::single(CdefFrameParams {
...
/// libaom `CDEF_ADAPTIVE` arm 2, searched path (pickcdef.c:1032-1091):
/// at mapped qindex <= 220 every picked strength's primary AND secondary
/// components are halved (`>> 1` on each packed component, so odd values
/// lose more than half — aom's Note 1 keeps that asymmetry). With
/// `zero_low_cdef_strengths` (aom `base_qindex <= 140`, speed_features.c:
/// 2886-2891) each entry is then zeroed: the luma entry when its halved
/// components satisfy `pri <= 4 && sec <= 1`, and the chroma entry when
/// the luma entry was low OR its own halved components were.
///
/// Operates on the packed `pri * 4 + sec` domain that
/// [`CdefPick::strengths`] carries — the same packing aom's
/// `cdef_strengths[]` uses. Strength-table indices (`fb_idx`) and
/// `cdef_bits` are untouched: aom keeps the table size even when every
/// entry zeroes, and the decoder runs a no-op CDEF pass in that case.
///
/// Zen extension — not a C-SVT state. [`CdefPick`] values produced by
/// the all-skip arm (all-zero already) pass through unchanged.
pub fn aom_adaptive_cdef_reduce(pick: &mut CdefPick, base_qindex: u8) {
    if base_qindex > 220 {
        return;
    }
    let halve = |s: u8| -> u8 { ((s / 4) >> 1) * 4 + ((s % 4) >> 1) };
    // aom's low test runs on the HALVED components: `new_pri <= 4 &&
    // new_sec <= 1` (pickcdef.c:1082-1088).
    let low = |s: u8| -> bool { s / 4 <= 4 && s % 4 <= 1 };
    let zero_low = base_qindex <= 140;
    for s in pick.strengths.iter_mut() {
        let ny = halve(s.0);
        let nuv = halve(s.1);
        s.0 = ny;
        s.1 = nuv;
...
/// libaom `CDEF_ADAPTIVE` on the `CDEF_PICK_FROM_Q` fast path
/// (`avoid_uv_cdef`, pickcdef.c:822-832/:1287): the chroma strength is
/// forced to 0 — luma keeps the polynomial pick, chroma is declared
/// unfiltered. Zen extension, same gate as [`aom_adaptive_cdef_reduce`].
pub fn aom_adaptive_cdef_avoid_uv(pick: &mut CdefPick) {
    for s in pick.strengths.iter_mut() {
        s.1 = 0;
    }
}
...
mod tests {
    ...
    /// libaom CDEF_ADAPTIVE arm 1: a mapped qindex <= 32 emits one
    /// all-zero strength with the qp-derived damping (pickcdef.c:846-857).
    #[test]
    fn aom_adaptive_off_pick() {
        let p = aom_adaptive_cdef_off_pick(32);
        assert_eq!(p.strengths, alloc::vec![(0u8, 0u8)]);
        assert_eq!(p.bits, 0);
        assert_eq!(p.damping, 3); // 3 + (32 >> 6)
        assert!(!p.any(false));
        let p = aom_adaptive_cdef_off_pick(20);
        assert_eq!(p.damping, 3);
    }

    /// libaom CDEF_ADAPTIVE arm 2 (pickcdef.c:1032-1091): strengths halve
    /// componentwise at qindex <= 220; at <= 140 the halved-low entries
    /// zero (`new_pri <= 4 && new_sec <= 1`, chroma when luma OR itself is
    /// low). Above 220 nothing happens.
    #[test]
    fn aom_adaptive_reduce_halves_and_zeroes() {
        let mut pick = CdefPick {
            damping: 5,
            bits: 0,
            // y: pri 12 sec 2 -> 12*4+2 = 50; uv: pri 10 sec 1 -> 41.
            strengths: alloc::vec![(50, 41), (31, 9)],
            fb_idx: alloc::vec![0, 1],
            nhfb: 1,
        };
        // qindex 200: halve, no zero_low. (50 -> pri 6/sec 1 = 25;
        // 41 -> pri 5/sec 0 = 20; 31 -> pri 3/sec 3 -> 3/... wait:
        // 31 = pri 7 sec 3 -> halved pri 3 sec 1 -> 13. 9 = pri 2 sec 1
        // -> halved pri 1 sec 0 -> 4.)
        aom_adaptive_cdef_reduce(&mut pick, 200);
        assert_eq!(pick.strengths, alloc::vec![(25, 20), (13, 4)]);

        // qindex 140: halve THEN zero the halved-low entries.
        // 50 -> halved (6,1) = 25: pri 6 > 4, luma not low -> keeps 25.
        // 41 -> halved (5,0) = 20: pri 5 > 4 -> keeps 20.
        // 31 -> halved (3,1) = 13: pri 3 <= 4 && sec 1 <= 1 -> low -> y=0;
        //   chroma 9 -> halved (1,0) = 4: low too -> uv=0 (also uv=0 via ly).
        let mut pick = CdefPick {
            damping: 5,
            bits: 0,
            strengths: alloc::vec![(50, 41), (31, 9), (31, 41)],
            fb_idx: alloc::vec![0],
            nhfb: 1,
        };
        aom_adaptive_cdef_reduce(&mut pick, 140);
        assert_eq!(
            pick.strengths,
            alloc::vec![(25, 20), (0, 0), (0, 0)],
            "entry 2's uv zeroes because its LUMA was low"
        );

        // qindex 230: above the <= 220 arm — the pick is untouched.
        let mut pick = CdefPick {
            damping: 6,
            bits: 0,
            strengths: alloc::vec![(50, 41)],
            fb_idx: alloc::vec![],
            nhfb: 0,
        };
        aom_adaptive_cdef_reduce(&mut pick, 230);
        assert_eq!(pick.strengths, alloc::vec![(50, 41)]);
    }

    /// `avoid_uv_cdef`: chroma strength zeroed, luma preserved.
    #[test]
    fn aom_adaptive_avoid_uv_zeroes_chroma_only() {
        let mut pick = CdefPick {
            damping: 4,
            bits: 0,
            strengths: alloc::vec![(43, 37)],
            fb_idx: alloc::vec![],
            nhfb: 0,
        };
        aom_adaptive_cdef_avoid_uv(&mut pick);
        assert_eq!(pick.strengths, alloc::vec![(43, 0)]);
    }
}
