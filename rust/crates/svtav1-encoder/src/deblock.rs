...
/// the frame level per (plane, direction) — C hardwires
/// `delta_lf_present = 0` (resource_coordination_process.c:434-441), so
/// there is no per-block delta.
///
...
/// (aom `set_lpf_parameters` av1_loopfilter.c:450-490; the two are equal
/// without a per-block delta, but the decoder's rule is kept whole). Returns (advance in
...
fn filter_plane(
    ...
    for my in 0..mi_rows_p {
        let mut mx = 0;
        while mx < mi_cols_p {
            let levels_v = (level_vert, level_vert);
            let (adv, filter) =
    ...
    for mx in 0..mi_cols_p {
        let mut my = 0;
        while my < mi_rows_p {
            let levels_h = (level_horz, level_horz);
            let (adv, filter) =
