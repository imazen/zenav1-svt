...
// carriers (`LeafGeom`, `ChromaCtx`, `LeafBd10`, `PalFlagRates`), runs the
// MDS1 luma loop inline, calls the stages in order, and picks the winner.
// MDS1 is the one stage still inline; it is ~170 lines and its inputs
// (`blk_crop`, the luma quant table) are the fields `LeafGeom` does not carry
// yet.
