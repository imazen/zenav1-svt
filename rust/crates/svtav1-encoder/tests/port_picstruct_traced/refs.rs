...
#[test]
pub(super) fn traced_set_frame_update_type_flat_gop() {
...
#[test]
pub(super) fn traced_prune_refs_fold_order() {
...
#[test]
pub(super) fn traced_update_dpb_masked_slots_only() {
...
#[test]
pub(super) fn traced_set_ref_list_counts_bwd_may_equal_last() {
...
#[test]
pub(super) fn traced_set_ref_list_counts_boosted_is_update_type_not_layer() {
...
#[test]
pub(super) fn traced_set_all_ref_frame_type_ordered_set() {
    let pic = pp::PicParams {
        slice_type: pp::SliceType::B,
...
#[test]
pub(super) fn traced_low_delay_flat_cqp_five_frames() {
    let seq = ld_flat_cqp_seq();
    let mut ctx = pp::PicDecisionCtx::default();
    ...
    pp::picture_decision_per_picture(&mut f0, &seq, &mut ctx, 0, 0, &|_, _| pp::INVALID_LUMA)
        .unwrap();
    ...
    pp::picture_decision_per_picture(&mut f1, &seq, &mut ctx, 0, 0, &|_, _| pp::INVALID_LUMA)
        .unwrap();
    assert_eq!(
        f1.update_type,
    ...
    pp::picture_decision_per_picture(&mut f2, &seq, &mut ctx, 0, 0, &|_, _| pp::INVALID_LUMA)
        .unwrap();
    assert_eq!(
        f2.update_type,
    ...
    pp::picture_decision_per_picture(&mut f3, &seq, &mut ctx, 0, 0, &|_, _| pp::INVALID_LUMA)
        .unwrap();
    ...
    pp::picture_decision_per_picture(&mut f4, &seq, &mut ctx, 0, 0, &|_, _| pp::INVALID_LUMA)
        .unwrap();
    assert_eq!(
        f4.update_type,
...
#[test]
pub(super) fn traced_long_base_ref_refresh_at_128() {
    let seq = ld_flat_cqp_seq();
    let mut ctx = pp::PicDecisionCtx::default();

    // POC 127: 127 - 0 < 128, so bit 7 is NOT set.
    let mut p127 = inter_frame(127, 0);
    pp::picture_decision_per_picture(&mut p127, &seq, &mut ctx, 0, 0, &|_, _| pp::INVALID_LUMA)
        .unwrap();
    ...
    pp::picture_decision_per_picture(&mut p128, &seq, &mut ctx, 0, 0, &|_, _| pp::INVALID_LUMA)
        .unwrap();
    ...
    pp::picture_decision_per_picture(&mut nb, &seq, &mut ctx, 0, 0, &|_, _| pp::INVALID_LUMA)
        .unwrap();
...
#[test]
pub(super) fn traced_random_access_flat_toggle_order() {
    ...
    pp::picture_decision_per_picture(&mut kf, &seq, &mut ctx, 0, 0, &|_, _| pp::INVALID_LUMA)
        .unwrap();
...
///   slots.
#[test]
pub(super) fn out_of_range_hierarchy_and_bad_mg_index_refuse() {
    let seq = pp::SeqPicParams {
        pred_structure: pp::PredStructure::RandomAccess,
        ..ld_flat_cqp_seq()
    };
    ...
    for hier in [6u8, 7, 255] {
        ...
        assert_eq!(
            pp::picture_decision_per_picture(&mut pic, &seq, &mut ctx, 0, 0, &|_, _| {
                pp::INVALID_LUMA
            }),
    ...
    let mut pic = inter_frame(1, 0);
    assert!(
        pp::picture_decision_per_picture(&mut pic, &ld, &mut ctx, 0, 0, &|_, _| pp::INVALID_LUMA)
            .is_ok()
    );
    ...
    pic.temporal_layer_index = 2;
    assert!(
        pp::picture_decision_per_picture(&mut pic, &seq, &mut ctx, 1, 0, &|_, _| pp::INVALID_LUMA)
            .is_ok()
    );
}
...
#[test]
pub(super) fn traced_rtc_flat_refresh_mask_covers_unused_slots() {
    let mut seq = ld_flat_cqp_seq();
    seq.rtc = true;
...
#[test]
pub(super) fn traced_set_frame_display_params() {
...
/// never written.
#[test]
pub(super) fn traced_set_ref_frame_sign_bias_index_spaces() {
    let seq = ld_flat_cqp_seq();
...
#[test]
pub(super) fn traced_circ_inc_dec() {
...
#[test]
pub(super) fn ref_index_constants_are_the_c_order() {
    assert_eq!(
