...
| kf_tf_strength / TF formula | **DONE + ACTIVE** (2026-09-24) — `derive_tf_shift` gained the `SVT_HDR_MODE` arm (`kf_tf_shift_factor = 10 + (4 - kf_tf_strength)`, `Option<u8>` = runtime mirror of the `#if`); `RaTfScs.kf_tf_strength` stamped `Some` only in fork mode; `SVT_FORK_KF_TF_STRENGTH` env reaches the RA pipeline (the multi-frame `identity_run` path was missing `pipeline.hdr = from_env()` — swept byte-identical until fixed). Fork-knob range checks added to `knob_config_error` mirroring `svt_av1_verify_settings` (:894-980). MODE1 e2e byte-parity NOT yet claimed — gated on the unconditional-delta campaign | `tf_traced.rs tier4_derive_tf_shift_fork_arm` pins both arms incl. adaptive-arm invariance; liveness measured: 9f RA gradient sweep s=0/1/3/4 → port key tile 8368/8171/7889/7877 B vs MODE1-C 7419/7386/7222/7250 B (same direction, knob live on both sides) |
