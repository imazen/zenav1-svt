...
- Low delay, real video: `tools/video_census_gate.sh` — 6 public-domain
  ...
  (`801d103f`), its preset -1 qp-scaler bits (`b0e7cbe5`), the p7 split
  walk's early exit (`aae52a01`) and the tx-size rate gated on
  `TX_MODE_SELECT` (`77d4d3ce`) made every KEY frame on the grid
  byte-identical (no TU0 cell remains). What is left is inter frames:
  0/2/3 diverge early (first difference in frame 1's loop-restoration taps,
  which come first in the tile, so the upstream decision is not yet
  known); plan 3.8 (b).
...
| monochrome | SUPERSEDED 2026-09-21 — `tools/mono_inter_gate.sh`: encoder recon == `aomdec` == `dav1d` on every frame across synthetic 13-px-shift legs x presets {0,6,8,13} x 64x64..256x128, sb64+sb128, a 6-frame inter chain, bd10 legs and the fourpeople clip, with anti-vacuity legs requiring real inter blocks + nonzero MVs. The corrupt stream the refusal cited predated the format-agnostic inter landings. Unaligned sizes verified 2026-09-25 (the whole measured defect table — 65x64, 64x67, 65x67, 66x66, 70x64, 64x70, 96x100, 100x96 — plus a 64x65 control, an unaligned 6-frame chain and sb128/bd10 legs). Mono qp0 inter still refuses — the mono lossless arm has no inter WHT residual path, so a qp0 "inter" stream is legal but silently all-intra (the gate pins the refusal) |
