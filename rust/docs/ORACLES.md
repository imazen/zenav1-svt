...
| `ghost-robot` | 65/288 (2026-09-26, after noise normalization was confined to non-bypass presets; every cell's verdict pinned by `tools/still_grid_gate.sh` in CI shard 1) | 869/873 on main at `35252c34` (2026-09-26, re-measured in the main checkout); the 4 open divergences are pinned in `oracles/divergent/ghost-robot.txt` and checked by `tools/cparity_ratchet.sh` in CI shard 1 |
