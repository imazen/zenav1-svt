...
| `crates/svtav1-encoder/src/pipeline.rs` | inter frames are shipped for 8-BIT 4:2:0, and this frame is above that. Measured against a DECODER, because that is what a wrong stream actually violates, 2026-09-11 on three public-domain derf clips x qp {20,40} x presets {6,8,10} x 4 frames at bit depth 10: only 8 of 18 cells reconstruct as aomdec does, the rest drifting from frame 1, 2 or 3. Encode video as 8-bit 4:2:0, or a single key frame at any depth and preset |
