...
104 regression cases and the archived probe test.
Probe and raw artifact records: `tools/perf_profile/dr_zone1_probe` and
`benchmarks/still_i265_2026-09-06-dr-z1-probe.*`.


The first V3 production A/B has99/99 identical pairs. NYC improves3.07% and
CLIC p2 improves2.43%, but CID p6/p8 regress0.65%/0.79%, CLIC p6 regresses
0.49%, and wiki regresses1.05%. Fresh same-binary controls also have99 identical
pairs and support the larger gains and these regressions. The candidate is
preserved in local jj change `xsyvusrz` and is not landed yet. Its text grows
1,676 bytes; assembly uses SIMD widening/multiply/shift/pack operations.

A follow-up30-group rectangle probe covers16x4 through64x32 and angles
3/14/45/67/87. Every candidate group is faster (smallest improvement about24%),
so small rectangular kernel cost does not explain the frame regressions.
Control biases and raw logs are retained; no code-layout cause is established.
A baseline-CPU version of the same contiguous loop is under investigation.
Initial frame records: `benchmarks/still_i265_2026-09-06-dr-z1-frame-{ab,control}.*`.
