> **SNAPSHOT — an inventory of ported-but-unwired code, dated in its own filename.** It describes the day it was written, not the current state. Current support is in
...
| 4 | **WARP HALF WIRED 2026-09-10 (`ddbfa257`)** — see `benchmarks/warped_motion_2026-09-10.meta`; the OBMC half and the MDS1 warp refinement remain. `port_md/motion_mode.rs` (12 of 13 fns) + `inter_me/obmc_search.rs` OBMC kernels (`obmc_sad`, `obmc_variance`, `get_obmc_mvpred_var`, `obmc_refining_search_sad`, `obmc_full_pixel_search`) | 2 files | `product_coding_loop.c:6741-6825` (`warp_refine_stage`/`obmc_refine_stage`/`opt_non_translation_motion_mode`), `:1068-1173` (`obmc_trans_face_off`), `entropy_coding.c:1159-1195` (`motion_mode_allowed`), `mode_decision.c:297-492` (`inter_intra_search`/`pick_interintra_wedge`), `av1me.c` OBMC search | **No** — only `warp_cand` (a small helper) has any non-test caller; everything else, including the whole OBMC motion-search kernel set, is dead | **MEASURED 2026-09-10 — YES, at preset 6.** `benchmarks/c_motion_mode_census_2026-09-10.meta` censuses C's own committed per-block record (`SVT_CINTER_OUT`) over the 24 `real_video_inter_gate.sh` cells: **88 of 1158 coded inter blocks are WARPED_CAUSAL (7.6 %)**, all at preset 6, none at preset 8, and OBMC is never selected. Every cell containing a warped block has frame 1 diverging in the gate's pinned table; the cells with none are the byte-identical ones. So this row is a CORRECTNESS gap in the tested envelope, not a wider-envelope RD question | Call `motion_mode_allowed` + the refine-stage functions from wherever inter MD candidates are generated (leaf_funnel or port_md/md_search.rs); wire the OBMC search kernels behind it | MOTION_MODE syntax + inter-intra compound are real AV1 features the candidate set is currently missing entirely on inter frames |
...
3. ~~Item 2 (global motion chain)~~ **Done 2026-09-10.** The whole chain is
   live: `global_me.c`'s derivation and search, `set_global_motion_field`,
   `write_global_motion`, the MVP walk's `gm_mv`, the GLOBALMV injector, the
   entropy walk's `gm_wmtype` and the predictor's `is_wm`. Gate:
   `tools/global_motion_gate.sh` (recon byte-identical to dav1d on the zoom
   cells where C fits a ROTZOOM). Wiring it also exposed and closed two
   defects that had nothing to do with global motion itself: the pack's
   `setup_ref_mv_list` passed a hardcoded zero `gm_mv`, and sub-8 inter chroma
   (`inter_chroma_4xn_pred`) was neither sized nor stitched as C does.
