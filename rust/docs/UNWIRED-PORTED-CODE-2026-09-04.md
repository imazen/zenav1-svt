...
| 6 | `port_md/nsq_skip.rs` general/inter-aware NSQ skip gates (`skip_by_split_rate`, `skip_by_sq_recon_dist`, `skip_by_shapes`, `skip_by_sq_txs`) vs `depth_refine.rs`'s all-intra-specialized private methods of the same name, which **are** live | `svtav1-encoder/src/port_md/nsq_skip.rs` (dead) vs `depth_refine.rs:1636+` (live) | `update_skip_nsq_based_on_split_rate` (`:9710`), `update_skip_nsq_based_on_sq_recon_dist` (`:9847`, **inter mode table at `:9867-9895`**: NEWMV/NEW_NEWMV shrink threshold 75%, NEAREST_NEARESTMV/NEAR_NEARMV double it, GLOBALMV/GLOBAL_GLOBALMV shift left 2), `update_skip_nsq_shapes` (`:9982`), `update_skip_nsq_based_on_sq_txs` (`:10063`) | `port_md/nsq_skip.rs` version: **no**. `depth_refine.rs`'s version: **yes**, called from `pipeline.rs` at 4+ sites (`:11147`, `:12084`, `:12346`, `:12363`, `:12668`+) | **Suspected — needs a differential test, not asserted as confirmed** | `depth_refine.rs:2058`'s own comment references a C call site conditioned on `slice_type != I_SLICE`, suggesting `depth_refine` already runs on inter frames — with the **all-intra** mode table, not the inter one `port_md/nsq_skip.rs` already carries. Confirm with a targeted inter-frame differential (compare `depth_refine`'s skip decision vs `port_md/nsq_skip.rs`'s on an inter block with NEWMV/GLOBALMV) before wiring | **This is the one candidate on this list that may be an ACTIVE correctness bug on inter frames right now**, not merely unused code — flagging above the rest of the list despite rank 6 for exactly that reason |
...
1. Item 6 (nsq_skip vs depth_refine) — confirm with a differential test
   FIRST, because if confirmed it's an active bug on already-shipped inter
   bytes, not a missing feature.
