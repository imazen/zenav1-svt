...
|---|---|---|
| Random-access / hierarchical GOPs | `port_picstruct::generate_rps_info` translates 4 of C's 8 pred-struct branches; `hierarchical_levels <= 5` accepted at the API and picture decision runs for every `intra_period != 1` (0 = key-once-then-inter, byte-verified vs C `intra_period_length` ≥ clip on flat-LDP) | port remaining pred-struct branches + RA reference management; largest single video item |
