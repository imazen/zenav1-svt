> Historical document from `rust/CLAUDE.md` at SVT `0cbd1279`. Original text follows unchanged. Status and priorities here are superseded by [the current handoff](https://github.com/imazen/zenav1-svt/blob/main/CONTEXT-HANDOFF.md). Original relative links and line numbers refer to the original location/revision.
...
The per-area prose below is hand-written and explains WHAT each area still owes;
the generated table above is the authoritative COUNT. If they disagree, the
table is right and the prose needs updating.
