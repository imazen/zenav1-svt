...
| **coeff-cost tx-type rate: the TWO-PART IBC quirk** | `rd_cost.c:392` + `md_rate_estimation.c:225-243` | **(a)** `svt_av1_cost_coeffs_txb` derives `is_inter = is_inter_mode(cand->block_mi.mode)` with **no `\|\| use_intrabc`** — unlike `tx_type_search` (product_coding_loop.c:4597) and the `av1_txt_rate_est` GATE, which both DO include it. An IntraBC txb therefore prices its tx type on the **INTRA** rows at `intra_dir = mode = DC_PRED`, while the WRITE path and the txt-search set/gate stay inter-classified. **(b)** `{intra,inter}_tx_type_fac_bits` are indexed by the RAW `TxType`, and `svt_aom_get_syntax_rate_from_cdf(..., av1_ext_tx_inv[set])` only ever writes that set's own members — so a query for a tx type OUTSIDE the row's set reads a literal **0**, not a symbol cost. Combined: an IBC 16x16 txb prices V_DCT/H_DCT at 0 (both are in the INTER 16x16 set `DTT9_IDTX_1DDCT`, neither is in the INTRA one, `DTT4_IDTX`). Port: `MdRates::txt_rate` (leaf_funnel.rs) needs BOTH halves — the `cost_dir` remap at the `plane_type == 0` call site AND an `AV1_EXT_TX_USED[set][tx_type] == 0 -> 0` membership guard, because the port's tables are SYMBOL-indexed and `AV1_EXT_TX_IND[set][out_of_set]` silently returns its 0 filler (= symbol 0 = IDTX, 2489 at qindex 255). MEASURED (gb82-sc graph.png 512x512 q63 p2, block mi(8,80) IBC 32x32, luma txb (16,0) 16x16): with both halves the txb costs 2808 == C; with only (a) it costs 5297, which flips the per-txb TXT winner to DCT_DCT/eob=0 where C codes V_DCT/eob=1 and desyncs the OBU at byte 160. Gate: `txt_rate_out_of_set_type_costs_zero_like_c`. |
