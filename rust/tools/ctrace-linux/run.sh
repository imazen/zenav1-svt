...
DUMP_KNOWN=(
    ...
    SVT_IFCOST_OUT SVT_PICKPART0_OUT SVT_REFSTATS_OUT SVT_QDELTA_OUT SVT_IFS_OUT
    SVT_PICKPART_MIROW SVT_PICKPART_MICOL SVT_CCOEF_XY SVT_CCOST_XY SVT_QLEVELS_XY
...
for v in SVT_CTREE_OUT SVT_PICKPART_OUT SVT_QLEVELS_OUT SVT_CCOEF_OUT \
    ...
    SVT_UVRATE_OUT SVT_PD0COST_OUT SVT_PD0CFG_OUT SVT_LFRECON_OUT SVT_LFRECON_BIN \
    SVT_FCTX_OUT SVT_CINTER_OUT SVT_HME_OUT SVT_INJCFG_OUT SVT_SUBPEL_OUT \
    SVT_IFCOST_OUT SVT_PICKPART0_OUT SVT_REFSTATS_OUT SVT_QDELTA_OUT SVT_IFS_OUT; do
    if [[ -n "${!v:-}" ]]; then
...
for v in SVT_PICKPART_MIROW SVT_PICKPART_MICOL SVT_CCOEF_XY SVT_CCOST_XY SVT_QLEVELS_XY \
    SVT_QLEVELS_COMP SVT_PART_MI SVT_CEDGE_XY SVT_FASTCOST_XY \
    SVT_FULLCOST_XY SVT_UVLOOP_XY SVT_UVRATE_XY SVT_PD0COST_SBY \
    SVT_IFCOST_XY; do
    [[ -n "${!v:-}" ]] && DUMP_ENV+=(-e "$v=${!v}")
...
for v in SVT_FRAMES SVT_AVIF SVT_INTRA_PERIOD SVT_HIER_LEVELS SVT_PRED_STRUCT \
    ...
    SVT_CRF_OFFSET SVT_CSP SVT_SUPERRES_KF_DENOM SVT_AQ_MODE; do
