...
for spec in "${CELLS[@]}"; do
    ...
    mkdir -p "$out"
    # SVTAV1_INTER_EXPERIMENTAL: harmless since the 2026-09-15 floor lift
    # (these cells used to run BELOW the preset-6 shipped floor); kept so the
    # run works on older binaries. See `crate::dbgenv::inter_experimental`.
