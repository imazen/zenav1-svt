...
# 2026-09-05: ALL 100 cells promoted per the self-promotion contract. The 78
# "pinned near-ties" were three MD-side mechanisms (an IntraBC candidate's
# tx-depth cap is C's INTRA cap; C's MD txfm-context stamp has no
# skip&&inter arm; C's MD-side context skips the palette CDF update for
# non-chroma-reference blocks) — commits 6891708c / c19c4f2f / 6df06356,
# measured 150/150 on gb82-sc x p0..4 x qp{20,40,48} by the byte-only twin
# `tools/screen_ibc_byte_gate.sh` (this gate's decode_diff oracle does not
# build off the CI image, so the promotion was measured with the twin and
# the map is benchmarks/screen_ibc_map_2026-09-05.txt).
BYTE_EXACT=(
  "codec_wiki_p0_q20"
  "codec_wiki_p0_q48"
  "codec_wiki_p1_q20"
