...
- **LOCALIZED:** first diverging op is op 1 — at a 32×32 node C codes PARTITION_SPLIT(3), the port codes PARTITION_VERT_4(9). The port legitimately picks a 4:1 partition (RD divergence — decode-conformance is not a byte-match gate), which at preset 0 (min_block_dim=4) produces sub-8 blocks (16×4, 4×16, 8×32). The design assumed min-8×8 ("sub-8x8 chroma-ref rules deferred", pipeline.rs:65/2429); the 4:1/AB partitions violate that. RULED OUT: the search-side `has_uv` (leaf_funnel.rs:2292) and pack-side `blk_has_uv` (pipeline.rs:2440) both implement `is_chroma_reference` identically and agree; the chroma PAIR geometry (cw/ch/cx/cy) is identical between funnel (:2323) and pack (:2447) and hand-traces correct for the 4:1 sub-8 pairs. So the desync is SUBTLER — the chroma TX size, the chroma coding ORDER for the pair, or a geometry edge case — and needs the EXACT desyncing block pinpointed (aomdec doesn't report position). This is the same area the prior session's ed066d197 ("sub-8 walk chroma pairing") attempted and left incomplete → genuinely hard.
...
- **NEXT (the fix pass):** pinpoint the desyncing block — decode the port's `rs.obu` with a position-reporting decoder or bisect by disabling 4:1/AB partitions, then compare the port's chroma tx-size + coding order for that block against the AV1 spec `is_chroma_reference` pair. Do NOT band-aid by disallowing 4:1 partitions on 420 (risks the 54/54 byte-matching primary path and is not C-faithful — C codes these correctly). This is the #1 gate per the mandate above.
