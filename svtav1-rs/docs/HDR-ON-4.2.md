...
| tune policies (IQ/MS_SSIM/FILM_GRAIN) | OPEN — need the tune plumbing itself (tune_ssim_level derivation per tune + slice, the alt r0 TPL variance for tune=SSIM). The SSIM MD machinery above + still_get_qmlevel (qm.rs) are ready for them | |
