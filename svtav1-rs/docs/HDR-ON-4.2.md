...
| alt_ssim_tuning, tune policies (IQ/MS_SSIM/FILM_GRAIN) | OPEN — inert at fork defaults (tune=PSNR). All require the tune-SSIM subsystem the port lacks entirely: per-candidate SSIM full cost through full_loop luma+UV (full_loop.c:2337 ssim_level plumbing), the two-pass SSD-then-SSIM winner selection (mode_decision.c:3875-3900), and the alt r0/variance TPL derivation (src_ops_process.c:2233). Multi-session subsystem — port it when tunes land, not as a fork knob one-off. still_get_qmlevel (the TUNE_IQ QM curve) is already ported in qm.rs for that day | classification verified against hdr/main + the hybrid |
